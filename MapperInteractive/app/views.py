...
first after loading the dataset from csv, first it will process the dataset (process_text_data) and save to './uplodas/processed_data.csv'
then it will load the graph data (get_graph) from './uploads/processed_data' and visualize

to guarantee this process can success, the original dataset should have header and no index
'''
...
def process_text_data():
    ...
    print(f"this is the new dataset shape {newdf1.shape}")
...
def load_mapper_data():
    ...
    print('finish loading mapper data')
...
def get_graph():
    ...
    # mapper data is readin columns together with hyperparameters
    ...
    # print('this is all cols: ', all_cols)
    # print('this is categorical cols: ', categorical_cols)

    data = pd.read_csv(APP_STATIC+"/uploads/processed_data.csv") # in this step, the webpage will save an csvfile to this address
    # then we read from this address and get the actual graph data
    ...
    print('finish getting mapper results')
...
def _call_kmapper(data, col_names, interval, overlap, clustering_alg, clustering_alg_params, filter_function, filter_parameters=None):
    ...
    print(f'max val {np.max(data_new)}')
    print(f'mean val {np.max(data_new)}')
    ...
    # print(f'lens dictionary: {lens_dict}')
...
def _parse_result(graph, lens_dict, data_array=[], if_cli=False):
    ...
    print('finished dumping node_detail')
...
def get_selected_data(selected_nodes):
    ...
    if len(selected_nodes) > 0:
        ...
        data = data.iloc[selected_rows, :]
        data.index = range(len(data))
    print(f"finish loading the data and mapper information")
    return data, cols
